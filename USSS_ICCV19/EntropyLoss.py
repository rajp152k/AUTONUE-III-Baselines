...
class EmbeddingLoss(torch.nn.Module):
	...
	def __init__(self , n_labels , embed_dim, label_embedding, loss_criterion):
		...
		self.conv_module_dict = torch.nn.Sequential(OrderedDict([
			...
			(self.datasets[3], torch.nn.Conv2d(embed_dim , label_embedding[self.datasets[3]].size(0) , kernel_size=1, bias=False)),
			(self.datasets[4], torch.nn.Conv2d(embed_dim , label_embedding[self.datasets[4]].size(0) , kernel_size=1, bias=False))
		]))
