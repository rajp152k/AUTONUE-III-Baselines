...
def update_tb(state,loss,score,loader_var,epoch,i,tb):
    '''
        update scalars in the logger
        Args:
            state: val/train
            loss: loss tuple(avg and immediate)
            score: score tuple(avg and immediate)
            epoch: current epoch
            i: iteration number in epoch
    '''
    # loss:0, score:2, state:1 :- order for formatted strings
    tb.add_scalar('{0}/{1}'.format("loss",state,"score"),loss.val,
            epoch*len(loader_var)+i)
    tb.add_scalar('{0}_avg/{1}'.format("loss",state,"score"),loss.avg,
            epoch*len(loader_var)+i)
    tb.add_scalar('{2}:accuracy/{1}'.format("loss",state,"score"),score.val,
            epoch*len(loader_var)+i)
    tb.add_scalar('{2}_avg:accuracy/{1}'.format("loss",state,"score"),score.avg,
            epoch*len(loader_var)+i)
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % print_freq == 0:
            update_tb("train",losses,scores,train_loader,epoch,i,tb)
...
def main():
    ...
    if args.cmd == 'train':
        global tb
        tb = SummaryWriter(Path(args.save_path)/'runs')
        train_seg(args)
    elif args.cmd == 'test':
        test_seg(args)
