...
def save_output_images(predictions, filenames, output_dir,level):
    """
    ...
    for ind in range(len(filenames)):
        ...
        new_name = filenames[ind].split("_")
        if level==2:
            fn = os.path.join(output_dir, new_name[0]+ "_gtFine_labellevel2Ids" + '.png')
        else:
            fn = os.path.join(output_dir, new_name[0]+ "_gtFine_labellevel3Ids" + '.png')
        out_dir = split(fn)[0]
...
def save_colorful_images(predictions, filenames, output_dir, palettes,level):
   """
   Saves a given (B x C x H x W) into an image file.
   If given a mini-batch tensor, will save the tensor as a grid of images.
   """
...
def test(eval_data_loader, model, num_classes,
         output_dir='pred', has_gt=False, save_vis=False,level=2):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    end = time.time()
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        batch_time.update(time.time() - end)
        if save_vis:
            save_output_images(pred, name, output_dir,level)
            save_colorful_images(
                pred, name, output_dir + '_color',
                TRIPLET_PALETTE if num_classes == 3 else CITYSCAPE_PALETTE,level)
        if has_gt:
            label = label.numpy()
...
def test_seg(args):
    ...
    if args.ms:
        ...
        mAP = test(test_loader, model, args.classes, save_vis=True,
                    output_dir=args.output_dir,level=args.level)
